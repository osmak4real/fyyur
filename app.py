...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:htmlcss.js@localhost:5432/fyyurproject'
  ...
  for area in all_areas:
    ...
    for venue in area_venues:
      venue_data.append({
        "id": venue.id,
        "name": venue.name, 
        "num_upcoming_shows": len(db.session.query(Show).filter(Show.venue_id==1).filter(Show.start_time>datetime.datetime.now()).all())
      })
    data.append({
      "city": area.city,
      "state": area.state, 
...
def show_venue(venue_id):
  ...
  past_shows_query = db.session.query(Show).join(Artist).filter(Show.venue_id==venue_id).filter(Show.start_time<datetime.datetime.now()).all()
  past_shows = []
...
def create_venue_submission():
  if request.method == "POST":
    ...
    db.session.commit()
    flash('Venue ' + request.form['name'] + ' was sucessfully listed!')
    db.session.close()
  else:
  ...
  # seach for "A" should return "Guns N Petals", "Matt Quevado", and "The Wild Sax Band".
   for result in search_result:
    data.append({
      "id": result.id,
      "name": result.name,
      "num_upcoming_shows": len(db.session.query(Show).filter(Show.artist_id == result.id).filter(Show.start_time > datetime.datetime.now()).all()),
    })
...
def show_artist(artist_id):
  ...
  past_shows_query = db.session.query(Show).join(Venue).filter(Show.artist_id==artist_id).filter(Show.start_time>datetime.datetime.now()).all()
  past_shows = []
  ...
  upcoming_shows_query = db.session.query(Show).join(Venue).filter(Show.artist_id==artist_id).filter(Show.start_time>datetime.datetime.now()).all()
  upcoming_shows = []
